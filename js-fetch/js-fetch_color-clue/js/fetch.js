...
export async function fetchNewColor() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const randomColorName = randomColor.name.value;
      const hexClosestToRandomColor = randomColor.name.closest_named_hex;
      setColorToGuess(hexClosestToRandomColor, randomColorName);
    } else {
